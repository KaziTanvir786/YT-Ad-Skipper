...
# Supported browser window title keywords.
# The active window title must contain "youtube" AND one of these browser names.
BROWSER_KEYWORDS = ["chrome", "firefox", "msedge", "edge", "opera", "brave", "vivaldi"]
...
def is_youtube_active():
    """
    Returns True only if the active window is a browser showing YouTube.
    Checks:
      1. Window title contains "youtube"
      2. Window title contains a known browser name
    This prevents clicking on any other app or webpage that has the word "Skip".
    """
...
class AdSkipperApp:
    ...
    def __init__(self, root):
        ...
        self._last_yt_warn   = 0.0   # Throttle "not YouTube" log messages
    ...
    def _build_window(self):
        ...
        self.root.geometry(f"380x540+{x}+{y}")
    ...
    def _build_ui(self):
        ...
        badge = tk.Label(top, text="⚡ Fast Mode", font=("Helvetica Neue", 9, "bold"),
                         fg=self.YELLOW, bg="#2a2000", padx=8, pady=3)
        ...
        yt_frame = tk.Frame(self.root, bg=self.CARD,
                            highlightbackground=self.BORDER, highlightthickness=1)
        yt_frame.pack(fill="x", padx=24)
        yt_inner = tk.Frame(yt_frame, bg=self.CARD)
        yt_inner.pack(fill="x", padx=18, pady=10)
        tk.Label(yt_inner, text="Window:", font=("Helvetica Neue", 9),
                 fg=self.GRAY, bg=self.CARD).pack(side="left")
        self.yt_status = tk.Label(yt_inner, text="● Waiting for YouTube",
                                  font=("Helvetica Neue", 9, "bold"),
                                  fg=self.GRAY, bg=self.CARD)
        self.yt_status.pack(side="left", padx=(6, 0))

        # ── Stats ──
        stats = tk.Frame(self.root, bg=self.BG)
        stats.pack(fill="x", padx=24, pady=(14, 0))
        ...
        # Start polling the window title for the YouTube indicator
        self._poll_yt_status()
    ...
    def _poll_yt_status(self):
        """Updates the YouTube window indicator in the GUI every 500ms."""
        if is_youtube_active():
            self.yt_status.config(text="● YouTube is active", fg=self.GREEN)
        else:
            title = get_active_window_title()
            if title:
                self.yt_status.config(text="● Not YouTube — scans paused", fg=self.YELLOW)
            else:
                self.yt_status.config(text="● Waiting for YouTube", fg=self.GRAY)
        self.root.after(500, self._poll_yt_status)
    ...
    def _start(self):
        self.running        = True
        self._last_click_at = 0.0
        self._last_yt_warn  = 0.0
        self.stop_event.clear()
        self._draw_toggle(True)
        self.status_label.config(text="● Active", fg=self.GREEN)
        self._log("Skipper started — watching for YouTube ads.", "green")
    ...
    def _scan_loop(self):
        while not self.stop_event.is_set():
            try:
                ...
                # ── Gate 1: Only run if YouTube is the active window ──
                if not is_youtube_active():
                    # Log a warning at most once every 5 seconds to avoid spam
                    now = time.time()
                    if now - self._last_yt_warn > 5:
                        self._last_yt_warn = now
                        title = get_active_window_title() or "unknown"
                        self.root.after(0, lambda t=title:
                            self._log(f'Paused — active window is not YouTube: "{t}"', "yellow"))
                    self.stop_event.wait(0.5)
                    continue

                # ── Gate 2: Cooldown after last click ──
                since_last_click = t_start - self._last_click_at
                if since_last_click < CLICK_COOLDOWN:
                    self.stop_event.wait(CLICK_COOLDOWN - since_last_click)
                    continue
